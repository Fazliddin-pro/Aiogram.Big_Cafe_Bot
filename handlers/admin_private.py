...
# Отправляем название новой категории
@admin_router.message(StateFilter(None), F.text == 'Добавить/Удалять категорию')
async def get_category_name(message: types.Message, state: FSMContext, session: AsyncSession):
    await message.answer("\n".join([
        "Напишите название категории.",
        "Если хотите удалить существующую категорию, напишите - 'del' и через пробел название.",
        "Если хотите добавить новую категорию, просто напишите её название."
    ]))
...
async def add_category(message: types.Message, state: FSMContext, session: AsyncSession):
    if message.text.startswith('del'):
        category_name = message.text.replace('del', '').strip()
        
        # Убедитесь, что функция возвращает корректный результат
        deleted_category = await orm_delete_category(session, category_name=category_name)
        if deleted_category:
            await message.answer('Категория и продукты в ней успешно удалены!')
        else:
            await message.answer('Такая категория не найдена.')
    else:
